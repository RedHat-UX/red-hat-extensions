...
const isElementSource = (x: string) => x && x.startsWith('rhx-') && !x.endsWith('.d.ts');
...
function getTagNameSlug(tagName: string) {
  const name = (pfeconfig?.aliases?.[tagName] ?? tagName).replace(`${pfeconfig?.tagPrefix ?? 'rhx'}-`, '');
...
function getTabData(filePath: string): ElementDocsPageTabData {
  ...
  const tagName = parts.find(x => x.startsWith('rhx-'))!;
  const pageSlug = filePath.split(sep).pop()?.split('.').shift()?.replace(/^(\d+|rhx)-/, '') ?? '';
  const slug = getTagNameSlug(tagName);
  const permalink =
        pageSlug === 'overview' ? `/elements/${slug}/index.html`
      : `/elements/${slug}/${pageSlug}/index.html`;
  const pageTitle = capitalize(deslugify(pageSlug).toLowerCase().replace('rhx-', ''));
  return {
    tagName,
    filePath,
    permalink,
